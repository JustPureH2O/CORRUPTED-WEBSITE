...
category: oi算法
published: 2024-03-15T16:16:46.781060+08:00
...
然后是前驱状态的问题，既然顺序是从下到上，那状态`dp[u][j]`就是从`dp[son(u)][j - 1]`转移来的，这里为什么是 $j-1$ ？因为根节点 $u$ 和这个子树的根节点 $\texttt{son}(u)$ 中间还连了一条边，因此需要将它排除在外。循环变量 $k$ 为上述数组第二维的值，用来枚举可选择的子树的边数。那么子树需要选择 $k$ 条边，算上刚刚的特殊边 $(u,\texttt{son}(u))$，根节点 $u$ 对应的树就只需要选择 $j-k-1$ 条边了。最后，因为选中了边 $(u,\texttt{son}(u))$ ，因此需要累加边权，状态转移方程就是 `dp[u][j] = max(dp[u][j], dp[u][j - k - 1] + dp[son(u)][j] + w[to[i]])`
